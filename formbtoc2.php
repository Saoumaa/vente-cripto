...
if(isset($_POST['valider'])){
        ...
        if($mpd == $mpd1){
          ...
          $updatebtoc = $bdd->prepare('UPDATE btoc set zip_adress = ?, city = ? , identity_card = ?,  where users_id =  ?');
        $updatebtoc->execute(array($adre,$city,$jcard,$_SESSION['id']));
        ...
        <form class="form-simple" id="form" method="POST">
            <h2 class="form-title">
              Formulaire d'inscription Btob
                ...
                <label for="identity-card"> Justificatif d'identité: (CI recto verso ou passeport)
                    <input type="file" name="justcard">
                </label>
