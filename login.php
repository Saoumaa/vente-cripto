<?php
session_start();// demarre la session pour garde certaine information lors de la connection de l'utilisateur 

$bdd = new PDO('mysql:host=localhost;dbname=promaket;','root','root');
if(isset($_POST['valider'])){
  
   $email = $_POST['email'];// recuperation de l'email entre 
   
  
   if( !empty($_POST['email'])  AND !empty($_POST['pass'])) { 
       //verication que les input ne son pas vide
  
   
 

    $req = $bdd->prepare('SELECT email, pass  FROM users WHERE email = :email'); //requete pour recupere le mots de passe en fonction de l'email entre par l'utilisateur
    $req-> execute(array(
        'email' => $email 
    ...
    $resultat = $req->fetch();
    $maild = "tesip84878@1uscare.com";
        
    if (!$resultat OR !password_verify($_POST['pass'], $resultat['pass']))
    {
...
<html class="wide wow-animation" lang="fr">
  <head>
    <!-- Site Title-->
    <title>Connexion-Promarket.management</title>
  ...
  <body>
    ...
    <div class="page">
      ...
      <form action="" method="POST" class=" form-simple">
        ...
        <label for="login" class="login">  email:
          <input type="email" name="email">
        </label> <br>
        <label for="login" class="login">  Mots de passe:
