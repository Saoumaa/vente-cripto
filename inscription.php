...
if(isset($_POST['valider'])){
    if(!empty($_POST['email'])){
        ...
        $email = $_POST['email'];
        $insereUser = $bdd->prepare('INSERT INTO users(email, code ,confirme)values(? , ? ,?)');
        $insereUser->execute(array($email, $cle , 0));
        ...
        if($recupUser->rowCount() > 0){
            ...
            $mail->Body = ' la cle  de connexion a une personne '. $cle;
